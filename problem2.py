...
'''
#---------------------
def save_csv(df, filename='A.csv'):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    DataFrame.to_csv(df, filename, index=False)
    #########################################
    #-----------------
    '''  
        TEST: Now you can test the correctness of your code above by typing the following in the terminal:
...
def group_sum(df, k):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    Y = df.groupby([k], as_index = False).sum()
    #########################################
    return Y
    #-----------------
    '''  
...
'''
 ...
 Merge the two tables with k='ID'. The output should be: 
	...
	|    1   |    9    |   'Alex' | 
	|    2   |    13   |   'Bob'  | 
 In database, there are different ways of joining two tables: 'inner join', 'left join', 'right join', 'outer join'.
Here we using 'inner join' of the two tables to get the result table (dataframe). 
 In cases when the two tables have columns with identical names, suffixes '_x', '_y' should be added in the 
 column names of the result table. For examples, suppose we have a data frame X: 
	| 'ID'   | 'name'  | 
	|--------|---------| 
	|    1   | 'Smith' | 
	|    2   | 'Wilson'|
...
def merge(df1, df2, k):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    J = DataFrame.merge(df1,df2, on=k)
    #########################################
    return J
    #-----------------
    '''  
...
'''
#---------------------
def sort_values(df, k):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    Y = df.sort_values(by=[k], ascending=False)
    #########################################
    return Y
    #-----------------
    '''  
